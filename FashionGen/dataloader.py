...
class TextDataset(data.Dataset):
    ...
    def load_captions(self, split, selected_categories):
        ...
        for i in range(caption_count):
            if file_h5['input_category'][i] in selected_categories:
                ...
                if len(tokens_new) > self.opt.WORDS_NUM:
                    word_count_overflow += 1

        print('%d captions exceeding the max word count of %d' % (word_count_overflow,  self.opt.WORDS_NUM))
    ...
    def get_caption(self, index):
        ...
        # pad with 0s (i.e., '<end>')
        x = np.zeros((self.opt.WORDS_NUM, 1), dtype='int64')
        x_len = num_words
        if num_words <= self.opt.WORDS_NUM:
            x[:num_words, 0] = sent_caption
        else:
            ix = list(np.arange(num_words))  # 1, 2, 3,..., maxNum
            np.random.shuffle(ix)
            ix = ix[:self.opt.WORDS_NUM]
            ix = np.sort(ix)
            x[:, 0] = sent_caption[ix]
            x_len = self.opt.WORDS_NUM
    ...
    def get_imgs(self, index):
        ...
        if self.opt.B_DCGAN:
            ...
            for i in range(self.opt.BRANCH_NUM):
                # print(imsize[i])
                if i < (self.opt.BRANCH_NUM - 1):
...
if __name__ == '__main__':
    ...
    opt = edit({
        'DATAROOT': '/home/OpenResource/Datasets/FashionGEN',
        'BATCH_SIZE': 8,
        'BASIC_SIZE': 256, # 64 for BRANCH_NUM == 3
        'BRANCH_NUM': 1,
        'WORDS_NUM': 10,
